...
![image](https://user-images.githubusercontent.com/58674365/102730429-1ad16880-4378-11eb-942b-c63e1e60fe73.png)<br>
...
FORM SET_DIRECTORY USING LS_KEY-OBJID.
  ...
  OBJFILE->DIRECTORY_BROWSE( EXPORTING  INITIAL_FOLDER = GV_INITIAL_DIR
                             ...
                             EXCEPTIONS CNTL_ERROR      = 1
                                        ...
                                        NOT_SUPPORTED_BY_GUI = 3 ).
  IF SY-SUBRC = 0.
    CONCATENATE GV_DIRECTORY '\' LS_KEY-OBJID '.xlsx' INTO GV_FILE.
  ELSE.
...
BACK단에다가 엑셀 파일을 불러들인 것, AXLM 메소드 내부도 OLE로 되어 있음, 업로드 시 열어두면 꼬일 수 도 있음

- SMW0: SAP SERVER에 저장, 이것도 CTS 태움

- AL11: SAP DIRECTORY 볼 수 있는 T-CODE

- axlm 같은 경우 컨트롤이 더 쉽지만 메모리 상의 문제로 6만건 정도만 한번에 올릴 수 있다. gui로 되는 메소드는 이러한 문제는 없지만 현업에서는 axlm을 더 많이 쓴다.

- at 구문을 사용하면, at 구문 이후의 character값은 다 * 난수) 값으로 바뀌어서 data handling이 불가, 일반적으로 지양, 만약 사용한다 하더라도 정확하게 sort한 후 사용

- 엑셀 데이터 업로드 할 때, 날짜 형식에 유의

  그대로 날짜 타입으로 받으면 오류가 날 수 있기에 char로 받은 후 가공하는 코드를 짜야함, 마찬가지로 화폐 콤마도 유의
...
at-end를 사용하는 것을 지양하기 위해, 엑셀 업로드 구문을 다음과 같이 변경할 수 있음. 그리고 at으로 특정 필드가 특정 값일 떄문 데이터를 불러오는 경우는 read table의 with key로 읽는 것이 더 편리
...
- DB에 저장된 값이 있는 것은 체크했지만, 업로드 하는 엑셀 자체에 중복된 값이 있는 것은 체크를 안했다는 것
- 
